...
class AuthController extends BaseController
{
	...
	public function actionLogin()
	{
		...
		if ($model->load(Yii::$app->request->post()) ) {
			...
			$user_designation = Yii::$app->db->createCommand('SELECT * FROM user WHERE username="' . $model->username . '" ')->queryOne();
		   
			if(!empty($user_designation['id'])){
				$designation = Yii::$app->db->createCommand('SELECT * FROM tbl_designation WHERE id="' . $user_designation['designation_id'] . '" ')->queryOne();
			}               
			// if(!($user_designation['designation_id']== 3) && !($user_designation['designation_id']== 4) ){
			if($designation['login']== 1 || $user_designation['superadmin']== 1 ){
				if($model->login()){
